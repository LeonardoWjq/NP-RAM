...
class ManiskillSingleDataset(Dataset):
    def __init__(self,
        ...
        super().__init__()
        assert obs_mode in [
            'state', 'rgbd'], f'Invalid observation mode {obs_mode}'
...
class ManiskillSeqDataset(Dataset):
    def __init__(self,
                 env_id: str,
                 obs_mode: str,
                 control_mode: str,
        ...
        with h5.File(data_path, 'r') as data:
            for key in tqdm(load_keys):
                ...
                obs = traj['obs']
                if obs_mode == 'state':
                    state = np.array(obs[:-1], dtype=np.float32)
                    state_sequences = obs_to_sequences(state, seq_len)
                    self.states.append(state_sequences)
                else:
                    state = np.hstack([flatten_state(obs['agent']),
                                       flatten_state(obs['extra'])])
                    state_sequences = obs_to_sequences(state[:-1], seq_len)
                    self.states.append(state_sequences)

                    image = obs['image']
                    rgbd = process_image(image)
                    rescaled_rgbd = rescale_rgbd(rgbd)
                    rgbd_sequences = obs_to_sequences(rescaled_rgbd[:-1], 
                                                      seq_len)
                    self.rgbds.append(rgbd_sequences)
...
if __name__ == '__main__':
    ...
    # print('Action Dimension:', dataset.act_dim)
    ...
    dataset = ManiskillSeqDataset(env_id,
                                  obs_mode,
                                  control_mode,
                                  seq_len=8,
                                  train=False)
