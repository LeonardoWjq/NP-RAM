...
def make_gail_callback(ckpt_path: str,
    ...
    def step_callback(step: int):
        if step % save_freq == 0:
            learner.save(os.path.join(ckpt_path, f"gail_sac_{step}"))
            learner.save(os.path.join(ckpt_path, f"gail_sac_latest"))
...
def train(seed: int = 0,
    ...
    if continue_training:
        learner_ckpt = os.path.join(ckpt_path, "gail_sac_latest")
        sac_learner = SAC.load(learner_ckpt)
        ...
        sac_learner = SAC(policy='MlpPolicy',
                          env=train_env,
...
if __name__ == "__main__":
    seeds = [3, 9, 7, 12, 4]
    for seed in seeds:
        train(seed=seed, total_time_steps=40_000)
        test(seed=seed)
